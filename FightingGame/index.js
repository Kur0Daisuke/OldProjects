...
function showControls() {
    ...
    document.querySelector('.popupBox').innerHTML = (`
            The controlButtons
            <br>
            Player 1 - WASD For movement<br>
            Player 2 - ArrowKeys For movement
            <br>
            for attacks 
            player 1 - spacebar<br>
            player 2 - ArrowDown
            <br>
            dash 
            double tap left or right keys for both players!

            <button class="exit" onclick="exit('mainScreen')">X</button>
    `)
    setTimeout(() => {
        document.querySelector('.popupBox').style.width = '80%'
        document.querySelector('.popupBox').style.fontSize = '30px'
        document.querySelector('.popupBox').style.height = '80%'
    }, 50)
...
function animate() {
    ...
    if(!healthDecreasing && enemy.health <= 0 || player.health <= 0) {
        determineWinner({player,enemy, timerId, mode})
    }
}
...
function keyCheck(key) {
    if(!player.dead) {
        switch(key) {
            ...
            break 
            case 'Shift':
            shuriken.start(player.position.x, player.position.y, 'right');
            break 
        }
    }
...
console.error(`you gay for opening this console, shame on you`)
